...
TRAIN_COMMAND='git config --global --add safe.directory /scratch/Mock_repo_mnlp && if [ ! -d "/scratch/Mock_repo_mnlp" ]; then git clone https://github.com/YoussefBL25/Mock_repo_mnlp.git /scratch/Mock_repo_mnlp && cd /scratch/Mock_repo_mnlp && git checkout youssef_branch; else cd /scratch/Mock_repo_mnlp && git checkout youssef_branch && git fetch origin && git reset --hard origin/youssef_branch; fi && cd /scratch/Mock_repo_mnlp && pip install -e promptomatix --no-deps && pip install --ignore-installed blinker && pip install dspy rouge langdetect backoff ujson litellm flask diffusers accelerate && chmod +x scratch/run_with_servers.sh && export LOCAL_VLM_URL="http://127.0.0.1:8000/v1/chat/completions" && export LOCAL_DIFFUSION_URL="http://127.0.0.1:8001/v1/images/generations" && export OPENAI_API_BASE="http://127.0.0.1:8000/v1" && ./scratch/run_with_servers.sh'
