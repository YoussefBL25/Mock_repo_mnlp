...
TRAIN_COMMAND='git config --global --add safe.directory /scratch/Mock_repo_mnlp && if [ ! -d "/scratch/Mock_repo_mnlp" ]; then git clone https://github.com/YoussefBL25/Mock_repo_mnlp.git /scratch/Mock_repo_mnlp; else cd /scratch/Mock_repo_mnlp && git remote set-url origin https://github.com/YoussefBL25/Mock_repo_mnlp.git && git fetch origin && git checkout master && git reset --hard origin/master; fi && cd /scratch/Mock_repo_mnlp && pip install -e promptomatix --no-deps && pip install blinker --ignore-installed && pip install anthropic dspy rouge langdetect backoff ujson litellm flask diffusers accelerate && cp scratch/run_with_servers.sh /tmp/run_with_servers.sh && bash /tmp/run_with_servers.sh'
