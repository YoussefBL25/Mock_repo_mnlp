...
class MetricsManager:
    ...
    def _get_output_value(item: Any) -> str:
        ...
        for field in fields:
            if isinstance(item, dict):
                values.append(str(item.get(field, '')).lower().strip())
            else:
                values.append(str(getattr(item, field, '')).lower().strip())
    ...
    def get_metrics_for_task(task_type: str) -> Callable:
        ...
        metrics_map = {
            ...
            'image_generation': MetricsManager._image_generation_metrics,
            'vqa': MetricsManager._vqa_metrics
        }
    ...
    def get_final_eval_metrics(task_type: str) -> Callable:
        ...
        metrics_map = {
            ...
            'image_generation': MetricsManager._image_generation_metrics_final_eval,
            'vqa': MetricsManager._vqa_metrics_final_eval
        }
    ...
    @staticmethod
    def _calculate_length_penalty(example: Any, instructions: Any = None) -> float:
        """Calculate exponential length penalty including static image tokens."""
        prompt_length = len(instructions.split()) if instructions else 0
        
        # Add Vision Token Economics
        has_image = False
        if hasattr(example, 'image_context') and getattr(example, 'image_context', None):
            has_image = True
        elif isinstance(example, dict):
            has_image = any(k in example and example[k] for k in ['image', 'images', 'image_url', 'image_path', 'image_context'])
            
        if has_image:
            # Assume 258 tokens per image (e.g., standard Gemini/OpenAI base cost)
            prompt_length += 258
            
        lambda_penalty = LambdaPenalty.get_value()
        return math.exp(-lambda_penalty * prompt_length)
    ...
    def _qa_metrics(example: Any, pred: Any, instructions: Any = None, trace: Any = None) -> float:
        ...
        try:
            ...
            # calculate length penalty with vision economics
    ...
    @staticmethod
    def _image_generation_metrics(example: Any, pred: Any, instructions: Any = None, trace: Any = None) -> float:
        """
        Calculates image generation metrics via local VLM-as-a-Judge and local Diffusion APIs.
        Supports automatic robust mock fallback if local APIs are offline.
        """
        try:
            import json
            import base64
            ...
            try:
                ...
                # Simulate evaluation score by analyzing prompt descriptive matching and length constraints
                desc_match_count = sum(1 for word in concept_text.lower().split() if word in candidate_prompt.lower())
                alignment_ratio = desc_match_count / max(1, len(concept_text.split()))
            ...
            length_penalty = MetricsManager._calculate_length_penalty(example, instructions)
            return float(score * length_penalty)
            
        except Exception as e:
            print(f"Error in image generation metrics: {str(e)}")
            return 0.0

    @staticmethod
    def _image_generation_metrics_final_eval(example: Any, pred: Any, instructions: Any = None, trace: Any = None) -> float:
        """Final evaluation metrics for image generation tasks."""
        return MetricsManager._image_generation_metrics(example, pred, instructions, trace)

    @staticmethod
    def _vqa_metrics(example: Any, pred: Any, instructions: Any = None, trace: Any = None) -> float:
        """
        Calculates Visual Question Answering (VQA) correctness score.
        Compares predicted answer string with gold-standard answer using similarity.
        """
        try:
            pred_answer = MetricsManager._get_output_value(pred)
            gold_answer = MetricsManager._get_output_value(example)
            ...
            # Calculate word-level token overlap similarity
            ...
            score = (em_score + overlap) / 2.0
            
            # Apply length penalty with vision economics
            length_penalty = MetricsManager._calculate_length_penalty(example, instructions)
            return float(score * length_penalty)
        except Exception as e:
            print(f"Error in VQA metrics: {str(e)}")
            return 0.0

    @staticmethod
    def _vqa_metrics_final_eval(example: Any, pred: Any, instructions: Any = None, trace: Any = None) -> float:
        """Final evaluation metrics for VQA tasks."""
        return MetricsManager._vqa_metrics(example, pred, instructions, trace)
