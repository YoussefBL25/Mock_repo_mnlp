...
$TrainCommand = 'git config --global --add safe.directory /scratch/Mock_repo_mnlp && if [ ! -d "/scratch/Mock_repo_mnlp" ]; then git clone https://github.com/YoussefBL25/Mock_repo_mnlp.git /scratch/Mock_repo_mnlp && cd /scratch/Mock_repo_mnlp && git checkout youssef_branch; else cd /scratch/Mock_repo_mnlp && git checkout youssef_branch && git pull; fi && cd /scratch/Mock_repo_mnlp && pip install -e promptomatix --no-deps && pip install dspy rouge langdetect backoff ujson litellm && chmod +x scratch/run_with_servers.sh && ./scratch/run_with_servers.sh'
